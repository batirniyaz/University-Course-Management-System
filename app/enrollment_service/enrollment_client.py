...
from app.course_service.course_pb2_grpc import CourseServiceStub
...
async def get_student(current_user: UserDep):
...
StudentDep = Annotated[dict, Depends(get_student)]
...
async def validate_course(enrollment_id: int):
    try:
        async with grpc.aio.insecure_channel("127.0.0.1:50052") as channel:
            stub = CourseServiceStub(channel)
            try:
                response = await stub.GetCourseById(course_pb2.GetCourseByIdRequest(id=enrollment_id))
                if not response:
                    raise HTTPException(
                        status_code=404,
                ...
                return {"course_id": response.id, "title": response.title, "description": response.description}
            except grpc.RpcError as e:
                raise HTTPException(
                    status_code=500,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Cannot connect to course service: {str(e)}"
        )
...
async def get_user_id(user_id: int):
    try:
        async with grpc.aio.insecure_channel("127.0.0.1:50051") as channel:
            stub = UserServiceStub(channel)
            try:
                response = await stub.GetUserById(user_pb2.GetUserByIdRequest(id=user_id))
                return {'user_id': response.id, 'username': response.username, 'role': response.role}
            except grpc.RpcError as e:
